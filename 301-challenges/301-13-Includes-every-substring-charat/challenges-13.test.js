...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(item=>item.match(/\d/gi).join(''));
...
const allHappy = (arr) => {
  // Solution code here...
  if (arr.filter(item=>item.includes(':)')).length===arr.length){
    return true;
  } else {
    return false;
  }
...
const findEvery = (arr, target) => {
  // Solution code here...
  if (arr.filter(item=>item.includes(target)).length===arr.length){
    return true;
  } else {
    return false;
  }
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(item=>item.filter(item2=>item2.indexOf('Brook')===-1));
};
...
const sortByDay = (arr) => {
  ...
  for (let i in daysOfWeek){
    newArr.push(arr.filter(item=>item.includes(daysOfWeek[i]))||[]);
  }
  return newArr;
};
