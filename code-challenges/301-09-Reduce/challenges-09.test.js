...
const reversedString = (str) => {
  ...
  let newArr = strArr.reduce((accu, item, i)=>{
    accu.push(strArr[strArr.length-1-i]);
    return accu;
  },[]);
  return newArr.join('');
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let totalNum = arr.reduce((accu,item,i)=>{
    if (item.children !== undefined){
      accu += item.children.length;
    }
    return accu;
  },0);
  return totalNum;
...
const calculateAverage = (arr) => {
  // Solution code here...
  let obj= arr.reduce((accu,item,i)=>{
    accu['count'] += 1;
    accu['sum'] += item;
    return accu;
  },{count: 0, sum: 0});
...
const extractChildren = (arr) => {
  // Solution code here...
  let filteredArr = arr.filter(item => (/a/gi).test(item.name));
  return filteredArr.reduce((accu,item,i)=>{
    if (item.children !== undefined){
      item.children.forEach(item=> accu.push(item));
    }
    return accu;
  },[]);
};
