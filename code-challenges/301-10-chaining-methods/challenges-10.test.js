...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((accu,item,i)=>{
    accu += item.filter(item => item === target).length;
    return accu;
  },0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((accu,item,i)=>{
    accu += item.reduce((accu2,item2,i)=>{
      accu2 += item2;
      return accu2;
    },0);
    return accu;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return  input.map(item =>{
    return item.filter(item2 => typeof(item2) ==='number' && item2 % 5 ===0).map(item3 => 2 ** item3);
  });
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((accu,item,i) => {
    if(item.name.match(/[a-zA-z]/gi).length < accu.match(/[a-zA-z]/gi).length){
      accu = item.name;
    }
    return accu;
  },data[0].name);
