...
let biggerThanLuke = (arr) => {
  ...
  // Number(arr.filter(luke=>luke.name==='Luke Skywalker')[0].mass)).join(' ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let y=0; y<3;y++){
    let colChecker='';
    for (let x=0; x<3; x++){
      colChecker += board[x][y];
    }
    if (colChecker==='XXX' || colChecker==='OOO'){
      return true;
    }
  }
  // check diagonal
  let diagChecker ='';
  for (let d=0; d<3;d++){
    diagChecker += board[d][d];
  }
  if (diagChecker==='XXX' || diagChecker==='OOO'){
    return true;
  }
  // check diagonal
  let diagChecker2 = '';
  for (let l=0;l<3;l++){
    let m;
    if(l===0){m=2;} else if(l===1){m=1;} else if(l===2){m=0;}
    diagChecker2 += board[l][m];
  }
  if (diagChecker2==='XXX' || diagChecker2==='OOO'){
    return true;
  }
