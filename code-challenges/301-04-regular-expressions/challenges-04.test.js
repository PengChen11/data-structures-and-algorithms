...
const citiesAtoJ = (arr) => {
  ...
  for (let i=0;i<arr.length;i++){
    let compair = /^[A-J]\w*/g;
    if (arr[i].match(compair) !== null){
      newArr.push(String(arr[i].match(compair)));
    }
  }
...
const matchMonth = (input) => {
  // Solution code here...
  let compair1 = /^(oct|Oct)/g;
  let compair2 = /\w*er$/g;
  let compair3 = /\w*ct$/g;
  if (String(input).match(compair1)!== null && (String(input).match(compair2)!== null || (String(input).match(compair3)!== null))) {
    return true;
  } else {
    return false;
  }
...
let hangman = (str) => {
  // Solution code here...
  let compair = /(a|e|i|o|u)/gi;
  return str.replace(compair,'_');
};
