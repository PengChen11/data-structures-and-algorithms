...
const sortSchedule = (arr) => {
  // Solution code here...
  arr.sort((a,b) => {
    let time1 = Date.parse(a.start);
    let time2 = Date.parse(b.start);
    let dur1 = Date.parse(a.end) - Date.parse(a.start);
    let dur2 = Date.parse(b.end) - Date.parse(b.end);
    if (a.dayOfWeek === b.dayOfWeek && time1 > time2){
      return 1;
    } else if (a.dayOfWeek === b.dayOfWeek && time1 < time2){
      return -1;
    }
    if (a.dayOfWeek === b.dayOfWeek && time1 === time2 && dur1 < dur2) {
      return 1;
    } else if (a.dayOfWeek === b.dayOfWeek && time1 === time2 && dur1 > dur2){
      return -1;
    }
    return 0;
  });
