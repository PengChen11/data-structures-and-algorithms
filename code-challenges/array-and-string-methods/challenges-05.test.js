...
const howMuchPencil = (str) => {
  ...
  // if I can't use .slice().

  // for (let i=0; i<str.length+1; i++){
  //   var sliceArr =[];
  //   for (let b=i; b<str.length;b++){
  //     sliceArr.push(str[b]);
  //   }
  //   result.push(sliceArr.join(''));
  // }
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  let searchArr = recipe.ingredients;
  for (let i=0; i<recipe.ingredients.length;i++){
    let firstIndex = searchArr[i].indexOf(' ');
    let secondIndex = searchArr[i].indexOf(' ', firstIndex+1);
    result.push(searchArr[i].slice(secondIndex+1));
  }
...
const stepActions = (recipe) => {
  ...
  for (let i=0; i<stepArr.length; i++){
    let newStepArr = stepArr[i].split(' ');
    result.push(newStepArr[0]);
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters<=0){
    ...
    for (let i=0; i<str.length-numberOfCharacters;i++){
      strArr.push(str[i]);
    }
  }
  return strArr.join('');
...
const removeVowels = (str) => {
  // Solution code here...
  let compair = /[^aeiou]/gi;
  return str.match(compair).join('');
};
...
const extractVowels = (str) => {
  // Solution code here...
  let newArr = [];
  let noVowels = /[^aeiou]/gi;
  let vowels = /a|e|i|o|u/gi;
  newArr.push(str.match(noVowels).join(''));
  newArr.push(str.match(vowels).sort().join(''));
  return newArr;
};
